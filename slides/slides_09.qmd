...
Violent Outcomes Across Ideological Movements

```{r w9-ideology-violence, warning=FALSE, message=FALSE}
library(tidyverse)
library(readxl)
library(plotly)
...
week9 <- PIRUS %>%
  mutate(
    ideology = case_when(
      Radicalization_Islamist == 1 ~ "Islamist",
  ...
  group_by(ideology) %>%
  summarise(
    violent_cases = sum(Violent == 1, na.rm = TRUE),
    total_cases = n(),
    violent_share = violent_cases / total_cases
  )
...
layout(
  ...
  yaxis = list(
      title = list(
        text = "Share of Violent Individuals",
        font = list(size = 20)
      ),
      tickfont = list(size = 18)
    )
