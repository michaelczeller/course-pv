...
# Poll: election-related violence {background-color="#2c844c"}

:::: {.columns}
:::{.column width="35%"}
![](slide_files/08/PVclass8_poll_QR.png){fig-alt="A QR code for the survey."}

:::

::: {.column width="65%" .center}
<!-- go to <https://www.qr-code-generator.com/>, add the link, and screen shot the QR code -->
Take the survey at **<https://forms.gle/aFgvV521ZFGC8Rih9>**

:::
:::: 

::: {style="margin-top: -20px;"}
[[Questions about groups]{style="color:violet;"}: (1) Does the group's violence increase around elections? (2) How is the group's violence connected to elections? (3) Has the group intimidated/targeted voters? (4) Has the group intimidated/targeted candidates/office-holders?]{style="font-size: 0.7em;"}
...
```{ojs}
md`## Poll results (Respondents: ${respondentCount}) - election violence`
```
...
surveyResults = liveGoogleSheet(
  "https://docs.google.com/spreadsheets/d/e/" +
    "2PACX-1vTX7RA8W_OCk1yCnikyaDz7WbZBF8AlFLTXcKm81JRCt_2Jyny_MoV0gEIqttuCJFcTrFN6S95B_nEV/" +
    "pub?gid=1764934299&single=true&output=csv",
...
Does the group's violence increase around elections?

```{ojs}
// need to replace 'g_vio_increase_elections'

g_vio_increase_electionsCounts = aq.from(surveyResults)
  .groupby("g_vio_increase_elections", "group")
  .count()
  .objects();

plot_g_vio_increase_elections = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_vio_increase_electionsCounts, {
      fx: "g_vio_increase_elections",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
## Group responses - election violence

How is the group's violence connected to elections?

```{ojs}
// need to replace 'g_connection_to_elections'

g_connection_to_electionsCounts = aq.from(surveyResults)
  .groupby("g_connection_to_elections", "group")
  .count()
  .objects();

plot_g_connection_to_elections = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_connection_to_electionsCounts, {
      fx: "g_connection_to_elections",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
## Group responses - election targeting 

:::: {.columns}
:::{.column width="48%"}
group intimidated/targeted voters?

```{ojs}
// need to replace 'g_target_voters'

g_target_votersCounts = aq.from(surveyResults)
  .groupby("g_target_voters", "group")
  .count()
  .objects();

plot_g_target_voters = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_target_votersCounts, {
      fx: "g_target_voters",
      x: "group",
      y: "count",
      fill: "group",
...
::: {.column width="48%"}
group intimidated/targeted candidates/office-holders? 

```{ojs}
// need to replace 'g_target_candidates'

g_target_candidatesCounts = aq.from(surveyResults)
  .groupby("g_target_candidates", "group")
  .count()
  .objects();

plot_g_target_candidates = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_target_candidatesCounts, {
      fx: "g_target_candidates",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
# Open data - Big Allied and Dangerous {background-color="#ffe0b2"}

::: panel-tabset
### Visualisation

Criminal Activity and Conflict Intensity of Insurgent Organisations

```{r w8-baad-profile, warning=FALSE, message=FALSE}
library(tidyverse)
library(readxl)
library(plotly)

baad <- read_excel("../resources/data/BAAD2_files/BAAD2 Insurgency Crime Dataset.xlsx")
...
plot_ly(
  ...
  text = ~paste(
    ...
    "<br><b>Country:</b>", hbase,
    "<br><b>Year:</b>", year,
    "<br><b>Ideology:</b>", ideology,
    "<br><b>Battle deaths:</b>", ucdpbd,
    "<br><b>Crime score:</b>", crime_score,
    "<br><b>Territorial control:</b>", territory
  ),
...
This scatter plot shows the relationship between criminal activity and conflict intensity among insurgent organisations in the [Big Allied and Dangerous Dataset](https://www.start.umd.edu/using-baad) (BAAD2). Each bubble is an organisation in a specific year. The x-axis shows a criminal activity score, which measures how many different forms of criminal financing an organisation uses, including drug trafficking, extortion, kidnapping, robbery, and smuggling. The y-axis shows the number of battle-related deaths associated with the conflict. Bubble size reflects organisational size; colours show different ideological types. By hovering over individual bubbles, you can view additional information about the organisation, the country in which it operated, the year of observation, and whether it exercised territorial control.
...
[Questions about groups next meeting]{style="color:violet;"}: (1) How does the group’s use of violence evolve over time? (2) How are major acts of violence perpetrated? (3) Does the group show restraint in choosing targets? (4) Is the group's restraint more strategic or moral? (5) Does the group have internal mechanisms to prevent uncontrolled violence? (6) Does the group escalate violence in response to state actions?  
