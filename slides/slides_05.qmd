---
title: "Class 5: Causes, Radicalisation, Mobilisation"
subtitle: "Radical subcultures and mobilisation"
author: "[Dr. Michael C. Zeller](https://michaelzeller.de)"
format: 
  rladies-revealjs:
    footer: ""
    slide-number: c/t
    toc: TRUE
    toc-depth: 1
    toc-title: Agenda for the day
    menu: TRUE
    theme: [default, custom_options]
    background-transition: fade
incremental: false
embed-resources: true
# runtime: shiny
bibliography: ../../../library.bib
---


# Opening notes {background-color="#2c844c"}

::: {.tenor-gif-embed data-postid="14209777" data-share-method="host" data-aspect-ratio="1.94" data-width="70%"}
<div class="tenor-gif-embed" data-postid="14209777" data-share-method="host" data-aspect-ratio="1.94" data-width="100%"><a href="https://tenor.com/view/open-gif-14209777">Open Sticker</a>from <a href="https://tenor.com/search/open-stickers">Open Stickers</a></div> <script type="text/javascript" async src="https://tenor.com/embed.js"></script> 
:::

```{=html}
<script type="text/javascript" async src="https://tenor.com/embed.js"></script>
```

## Follow-up from last week 

- factors shaping politically violent groups' [strategy]{style="color:darkred;"}
    + multiple simultaneously 
    + some more based around 'rational' calculation (e.g., available resources, state posture); others less so (e.g., ideology, emotional dynamics) 

. . . 

- [reminder]{style="color:red;"}: cases discussed in course are meant to help clarify theory and concepts

> a great deal of charged politics behind many of these cases---but the justifiability of causes in these cases is not part of this course  

## Presentation groups

<!-- ::: panel-tabset -->
<!-- ## December -->


| Date    | Presenters                       | Method |
| -------:|:---------------------------------|:------:|
|         |                                  |        | 

: Presentations line-up

<!-- | Date    | Presenters                       | Method | -->
<!-- | -------:|:---------------------------------|:------:| -->
<!-- | 4 Dec:  | Shahadaan, Kristine, Daichi      | ethnography |  -->
<!-- | 11 Dec: | Bérénice, Zorka, Victoria, Katharina | TBD    |  -->
<!-- | 18 Dec: | Shoam, Aidan, Tara, Sebastian    | QCA    |  -->

<!-- ## January -->

<!-- | Date    | Presenters                       | Method | -->
<!-- | -------:|:---------------------------------|:------:| -->
<!-- | 8 Jan:  |                                  | TBD    |  -->
<!-- | 15 Jan: |                                  | TBD    |  -->
<!-- | 22 Jan: |                                  | TBD    |  -->
<!-- | 29 Jan: |                                  | TBD    |  -->

<!-- : Presentations line-up -->

<!-- ::: -->

<!-- []{style="font-size:10px;"} -->

# Klausur preview follow-up {background-color="#2c844c"}

:::: {.columns}
:::{.column width="50%"}
- structure of an essay
    1. Broad introductory response
    2. Elaborate in (sufficient) detail to answer the questions
    3. Describe examples
    4. Concluding summary

**[any follow-up questions on this? (full review in late January)]{style="color:red;"}**

:::

::: {.column width="50%" .right}
<div class="tenor-gif-embed" data-postid="21422908" data-share-method="host" data-aspect-ratio="1" data-width="100%"><a href="https://tenor.com/view/mr-bean-benestad-alvesta-exam-mr-bean-exam-gif-21422908">Mr Bean Benestad GIF</a>from <a href="https://tenor.com/search/mr+bean-gifs">Mr Bean GIFs</a></div> <script type="text/javascript" async src="https://tenor.com/embed.js"></script> 
:::
::::


# Subculture {background-color="#2c844c"}

:::: {.columns}
:::{.column width="50%"}
- definition 
- example: from ska to skinhead
- other subculture examples 
- explanatory power: IS foreign fighters

:::

::: {.column width="50%" .right}
<div class="tenor-gif-embed" data-postid="21841153" data-share-method="host" data-aspect-ratio="1.2749" data-width="100%"><a href="https://tenor.com/view/i-love-culture-culture-muppets-kermit-miss-piggy-gif-21841153">I Love Culture Culture GIF</a>from <a href="https://tenor.com/search/i+love+culture-gifs">I Love Culture GIFs</a></div> <script type="text/javascript" async src="https://tenor.com/embed.js"></script> 
:::
::::

## Subculture defined

::: {style="text-align: center;"} 
[[subculture]{style="color:darkred;"} - a cultural group within a larger culture with its own traits, beliefs, and interests, typically distinct from and sometimes at odds with the larger culture]{style="font-size:55px;"} 
::: 

## Subculture example - from ska to skinhead {auto-animate=true}

- 1960s: '[mod]{style="color:forestgreen;"}' subculture in UK (working class youth with upward social mobility, stylish clothes, [American soul]{style="color:violet;"} & [Jamaican ska]{style="color:goldenrod;"})
    + '[hard mods]{style="color:forestgreen;"}': short haircuts, jeans and boots---worker aesthetic
        * [skinheads]{style="color:forestgreen;"}: Jamaican 'rude boys' opposed to authority
- 1970s: skinhead aesthetic, 'street punk'/*Oi!* music
    + [right-wing skins]{style="color:forestgreen;"}: rejection of black/Jamaican roots
        * 'Nazi rock': pioneering bands like *Screwdriver* (Ian Stuart Donaldson) and *Böhse Onkelz* [@Brown2004; @Dyck2016]
        * British-German connections
    + [left-wing skins]{style="color:forestgreen;"}: 'unpolitical' and clinging to Jamaican roots
        * S.H.A.R.P. (Skinheads Against Racial Prejudice)


::: {.r-stack}
![](slide_files/05/jamaican_ska.png){.fragment .absolute top=50 left=200}

![](slide_files/05/mod.png){.fragment .absolute top=50 left=50}

![](slide_files/05/mod_hard.png){.fragment .absolute top=50 left=200}

![](slide_files/05/skins.png){.fragment .absolute top=50 left=100}

:::


## Subculture example - from ska to skinhead {auto-animate=true}

- So this evolution has created a [subculture]{style="color:darkred;"}: music, clothing (e.g., shoelaces), political beliefs and orientation

- 1960s: '[mod]{style="color:forestgreen;"}' subculture in UK (working class youth with upward social mobility, stylish clothes, [American soul]{style="color:violet;"} & [Jamaican ska]{style="color:goldenrod;"})
    + '[hard mods]{style="color:forestgreen;"}': short haircuts, jeans and boots---worker aesthetic
        * [skinheads]{style="color:forestgreen;"}: Jamaican 'rude boys' opposed to authority
- 1970s: skinhead aesthetic, 'street punk'/*Oi!* music
    + [right-wing skins]{style="color:forestgreen;"}: rejection of black/Jamaican roots
        * 'Nazi rock': pioneering bands like *Screwdriver* (Ian Stuart Donaldson) and *Böhse Onkelz* [@Brown2004; @Dyck2016]
        * British-German connections
    + [left-wing skins]{style="color:forestgreen;"}: 'unpolitical' and clinging to Jamaican roots
        * S.H.A.R.P. (Skinheads Against Racial Prejudice)

## For those interested in a more recent example...

:::: {.columns}
:::{.column width="50%"}
A short documentary about the Rechtsrock concert in Ostritz in 2019:

:::

::: {.column width="50%" .right}
<!-- Aspect ratios include 1x1, 4x3, 16x9 (the default), and 21x9. -->
<!-- aspect-ratio="21x9"  -->
{{< video https://www.youtube.com/watch?v=ypxkGrrB99I 
    start="1"
    width="40%" 
    height="40%" >}}

:::
::::

But no alcohol allowed... (see further in [Spiegel](https://www.spiegel.de/politik/deutschland/ostritz-in-sachsen-buerger-kaufen-neonazis-das-bier-weg-a-1273834.html) article)

::: panel-tabset
## translated 
> Given the context of the other events and the resulting encounter between the various political camps, as well as the obviously combative and aggressive nature of the event, alcohol consumption would undoubtedly further increase the risk of violent clashes.

## Deutsch (original)

> Vor dem Hintergrund der weiteren Veranstaltungen und der damit einhergehenden Begegnung der verschiedenen politischen Lager sowie des offensichtlich kämpferisch-aggressiven Charakters der Veranstaltung würde ein Alkoholkonsum unzweifelhaft die Gefahr von gewaltsamen Auseinandersetzungen weiter erhöhen.

::: 

<!-- <https://www.spiegel.de/politik/deutschland/ostritz-in-sachsen-buerger-kaufen-neonazis-das-bier-weg-a-1273834.html> -->

<!-- ALSO: <https://www.youtube.com/watch?v=zKX9OjNy_NI> -->

## Open data - right-wing extremist codes {background-color="#ffe0b2"}

::: panel-tabset
### Data table 

```{r}
library(reactable)
library(tidyverse)

fash_fashion_de <- read_csv("../resources/data/FASHION_AGAINST_FASCISM_(DE).csv")
fash_fashion <- read_csv("../resources/data/FASHION AGAINST FASCISM.csv")

reactable::reactable(fash_fashion,
  defaultPageSize = 5,
  style = list(fontSize = "23px"),
  defaultColDef = colDef(
    align = "left",
    minWidth = 50,
    headerStyle = list(background = "#fabf68", fontWeight = "bold") # #f7f7f8
  ),
  columns=list(
    Code=colDef(name="Code", align="right", minWidth=70), # , maxWidth=200
    Description=colDef(name="Description", minWidth=300) 
  )
)
  
```

::: {.callout-note collapse="true" style="margin-top: -15px;"}
#### Data download (English)

```{r echo = F, collapse = TRUE, comment = "#>", message = FALSE, warning=FALSE}
library(downloadthis)

fash_fashion %>% download_this(
    output_name = "fash_fashion",
    output_extension = ".csv",
    button_label = "Download dataset as csv",
    button_type = "warning",
    has_icon = TRUE,
    icon = "fa fa-save"
  )
```

:::

::: {.callout-note collapse="true" style="margin-top: -15px;"}
#### Data download (German)

```{r echo = F, collapse = TRUE, comment = "#>", message = FALSE, warning=FALSE}
library(downloadthis)

fash_fashion_de %>% download_this(
    output_name = "fash_fashion_de",
    output_extension = ".csv",
    button_label = "Download dataset as csv",
    button_type = "warning",
    has_icon = TRUE,
    icon = "fa fa-save"
  )
```

:::


### Code similarity net 

```{r fash_fashion_netz, message=FALSE, warning=FALSE}

library(dplyr)
library(stringr)
library(tidytext)
library(tidyr)
library(purrr)
library(proxy)
library(visNetwork)

fash <- fash_fashion %>%
  mutate(
    Code = str_squish(Code),
    Description = str_squish(Description)
  )

## make categories for the codes
fash <- fash %>%
  mutate(
    Category = case_when(
      # NUMERIC ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "18", "28", "74", "84", "88",
        "124", "192", "311",
        "1488", "1888",
        "13/4/2007", "19/8",
        "68686", "168:1",
        "33/6", "4/19", "4/20",
        "44+44", "444",
        "111", "87+1",
        "1919", "191"
      ) ~ "numeric",
      # ORGANISATIONS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "Aryan Brotherhood", "AB",
        "AWB",
        "B & H", "B.B.N.", "BN",
        "C18", "318",
        "CI",
        "Firm 22",
        "Landvolkbewegung",
        "Freie Sachsen",
        "Hagida", "Hamgida",
        "HoGeSa", "HooNaRa", "Hoygida",
        "HS", "HSN",
        "IB",
        "Kagida",
        "KKK",
        "KO51 / Knockout 51",
        "Kögida",
        "Legida",
        "Magida",
        "Mvgida",
        "Pagan Front", "Heidnische Front",
        "proud boys",
        "NSB", "NSBA",
        "Wotan's folk", "Wotansvolk",
        "WAW",
        "Rusich Group",
        "Chetnik",
        "MHP",
        "Identitarian Movement",
        "NSAA",
        "3. Weg"
      ) ~ "organisation",
      # PERSONALITY ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "Varg Vikernes",
        "Count Grishnackh",
        "Elliot Rodger",
        "Makss Damage"
      ) ~ "personality",
      # MUSIC ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "Absurd",
        "B.E.D.", "BED",
        "Burzum",
        "NSBM", "NSHC",
        "RAC",
        "Mgla",
        "N'socialist Soundsystem",
        "essess"
      ) ~ "music",
      # MERCHANDISE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "AA",
        "BXNCNCTN", "BOX CON", "BC",
        "GRFVGL",
        "ISEGRIM",
        "S.T.O.G.",
        "Brachial",
        "Consdaple",
        "Dobermans Aggressive",
        "Erik & sons",
        "Homefront",
        "Masterrace",
        "Natural Instinct Germany",
        "One Family",
        "Phalanx Europe"
      ) ~ "merchandise",
      # SYMBOLS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "Bozkurtlar", "Ülkücü", "MHP", "Graue Wölfe",
        "HKNKRZ", "HKN KRZ",
        "I ♥ HK",
        "Triskele",
        "Doppelblitz"
      ) ~ "symbol",
      # MEME ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "FTWR", "YOTR", "NFTRH",
        "Great Awakening", "Großes Erwachen",
        "Kekistan", "Make Kekistan Great Again",
        "Moon Man",
        "NWO",
        "Pepe the Frog",
        "Q", "Q.Anon",
        "Red Pill", "Redpilling", "Red Piller",
        "Sheeple", "Schlafschaf",
        "The Great Reset",
        "WWG1WGA",
        "17Anon", "2q2q"
      ) ~ "meme",
      # SLOGANS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "All Lives Matter",
        "Black Rifles Matter",
        "Black Guns Matter",
        "Blut und Ehre",
        "Burn Loot Murder",
        "Deutsche wehrt euch!",
        "Eine Kugel reicht nicht",
        "FAFO",
        "Fuck Around and Find Out",
        "FCK ANTIFA",
        "FCK BLM",
        "FCK RFGS",
        "FCK MRKL",
        "FGRN",
        "From the River to the Sea",
        "GNLS",
        "HTLR SCHNTZL",
        "ITSUB",
        "KABARK",
        "KIGY",
        "Klagt nicht, kämpft",
        "KLASP",
        "Let's Go Brandon",
        "Lügenpresse",
        "Meine Ehre heißt Treue",
        "MEHT",
        "RAHOWA",
        "ROA",
        "RRR",
        "SGH",
        "Small HATS BIG problems",
        "Stolzmonat",
        "SWP",
        "TWITB",
        "UAO",
        "Ultra MAGA",
        "VTR LND",
        "W.A.P.",
        "W.O.T.A.N.",
        "WAR",
        "WBS",
        "Wenn dich diese Flagge stört, dann helfe ich dir packen.",
        "White Lives Matter",
        "WP",
        "WPWW"
      ) ~ "slogan",
      # DEROGATORY TERMINOLOGY ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "ASAB",
        "DDD",
        "Ernegl",
        "JOGGER",
        "Jewish Question",
        "JQ",
        "JOG",
        "N.A.Z.I.",
        "NTNL SZLST",
        "pureblood",
        "rapefugee",
        "Siege Pill",
        "Umvolkung"
      ) ~ "derogatory",
      # HISTORICAL / NAZI REFERENCES ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      Code %in% c(
        "Decima Flottiglia MAS",
        "KdF",
        "Kraft durch Freude",
        "Landser",
        "TWR",
        "TYP VII C",
        "Zyklon B"
      ) ~ "historical",
      TRUE ~ "other" # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    )
  )

# text prep ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
extra_stopwords <- tibble(
  word = c(
    "stands", "standing", "means", "meaning",
    "refers", "referring", "used", "use",
    "term", "name", "code", "acronym",
    "expression", "phrase", "slogan",
    "group", "movement", "popular",
    "right", "wing", "extreme", "extremist",
    "far", "german", "germany",
    "american", "us", "usa"
  )
)

words <- fash %>%
  mutate(id = row_number()) %>%
  select(id, Code, Description) %>%
  unnest_tokens(word, Description) %>%
  anti_join(stop_words, by = "word") %>%
  anti_join(extra_stopwords, by = "word") %>%
  filter(
    str_detect(word, "[a-z]"),
    str_length(word) > 2
  )

# ## check
# words %>% count(word, sort = TRUE) %>% slice_head(n = 30)

## calculate TF-IDF: gives each word a weight according to how informative it is
tfidf <- words %>%
  count(id, word) %>%
  bind_tf_idf(
    term = word,
    document = id,
    n = n
  )

# document-term matrix
tfidf_wide <- tfidf %>%
  select(id, word, tf_idf) %>%
  pivot_wider(
    names_from = word,
    values_from = tf_idf,
    values_fill = 0
  )

## calculate cosine similarity
tfidf_matrix <- tfidf_wide %>%
  arrange(id) %>%
  select(-id) %>%
  as.matrix()

similarity_matrix <- proxy::simil(
  tfidf_matrix,
  method = "cosine"
) %>%
  as.matrix()

## adjust this as needed
similarity_threshold <- 0.20

edges <- expand.grid(
  from = seq_len(nrow(similarity_matrix)),
  to   = seq_len(ncol(similarity_matrix))
) %>%
  filter(from < to) %>%
  mutate(
    similarity = similarity_matrix[
      cbind(from, to)
    ]
  ) %>%
  filter(similarity >= similarity_threshold)

## network prep ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
degree <- bind_rows(
  edges %>% select(node = from),
  edges %>% select(node = to)
) %>%
  count(node, name = "degree")

nodes <- fash %>%
  mutate(id = row_number()) %>%
  left_join(
    degree,
    by = c("id" = "node")
  ) %>%
  mutate(
    degree = replace_na(degree, 0)
  )

category_colours <- c(
  "other"         = "#9E9E9E",
  "merchandise"   = "#E69F00",
  "music"         = "#56B4E9",
  "slogan"        = "#009E73",
  "organisation"  = "#0072B2",
  "derogatory"    = "#D55E00",
  "symbol"        = "#CC79A7",
  "personality"   = "#F0E442",
  "historical"    = "#8C6BB1",
  "meme"          = "#66A61E",
  "numeric"       = "#A6761D"
)

nodes <- nodes %>% mutate(color = category_colours[Category])

nodes <- nodes %>%
  mutate(
    size = scales::rescale(
      sqrt(degree + 1),
      to = c(8, 30)
    )
  )

nodes <- nodes %>%
  mutate(
    label = Code,

    title = paste0(
      "<div style='max-width:350px; ",
      "font-family:Arial,sans-serif;'>",
      "<h3 style='margin-bottom:5px;'>",
      Code,
      "</h3>",
      "<p><strong>Category:</strong> ",
      Category,
      "</p>",
      "<p>",
      Description,
      "</p>",
      "<p><strong>Connections:</strong> ",
      degree,
      "</p>",
      "</div>"
    )
  )

edges_vis <- edges %>%
  mutate(
    width = scales::rescale(
      similarity,
      to = c(1, 5)
    ),
    title = paste0(
      "Similarity: ",
      round(similarity, 2)
    )
  )

## make vis 
network <- visNetwork(
  nodes,
  edges_vis,
  width = "100%",
  height = "850px"
) %>%

  visNodes(
    shape = "dot",
    borderWidth = 1,
    borderWidthSelected = 3,
    font = list(
      size = 14,
      face = "Arial"
    ),
    scaling = list(
      min = 8,
      max = 30
    )
  ) %>%

  visEdges(
    color = list(
      color = "#D0D0D0",
      highlight = "#555555",
      hover = "#777777"
    ),
    smooth = list(
      type = "continuous"
    ),
    selectionWidth = 3
  ) %>%

  visInteraction(
    hover = TRUE,
    navigationButtons = TRUE,
    keyboard = TRUE,
    multiselect = FALSE
  ) %>%

  visOptions(
    highlightNearest = list(
      enabled = TRUE,
      degree = 1,
      hover = TRUE
    ),
    nodesIdSelection = list(
      enabled = TRUE,
      useLabels = TRUE,
      main = "Find a code:"
    )
  ) %>%

  visPhysics(
    solver = "forceAtlas2Based",
    forceAtlas2Based = list(
      gravitationalConstant = -35,
      centralGravity = 0.01,
      springLength = 120,
      springConstant = 0.04,
      damping = 0.4,
      avoidOverlap = 1
    ),
    stabilization = list(
      enabled = TRUE,
      iterations = 1000,
      updateInterval = 100
    )
  )

network

```

::: {.fragment .absolute top=100 left=10}
Why are these two codes connected?  
Is the connection meaningful? 

::: 

### Explanatory note

This visualisation is drawn from '[Fashion against Fascism](https://fashionagainstfascism.com/en)' project of the '[Laut gegen Nazis](https://www.lautgegennazis.de/)' association. As part of the association's goal to stymie right-wing extremist activity (particularly in Germany), they support efforts to trademark certain symbols and codes, intending to prevent right-wing extremists from using them on merchandise, etc.. This denies them an important source of financing.

The network shows similarities in code descriptions in the dataset. (*Term Frequency-Inverse Document Frequency*: statistical measure of the importance of a word in a document relative to a corpus.) Each code is a node; nodes are coloured based on a quick categorisation; larger nodes have more connections; connections represent textual similarity (just based on the 'Description') between two codes. 

::: 

## Other subculture examples?

[Any [subculture]{style="color:darkred;"} examples you know? Any overlap with extremist groups? What are the distinguishing characteristics? Certain clothing, music, symbols, practices?]{style="color:indigo;"} 

. . . 

- From the right-wing extremist scene: Skinheads and [Rechtsrock](https://www.youtube.com/watch?v=ypxkGrrB99I), [football hooligans](https://www.youtube.com/watch?v=fzFDJOH1kcA), [Incels](https://www.youtube.com/watch?v=9p7dRa9-7as), sovereign citizens and [Reichsbürger](https://www.youtube.com/watch?v=aOjHs_pMbOg&pp=ygUNcmVpY2hzYsO8cmdlcg%3D%3D), neo-pagan, occultists, [military veterans (and active service personnel)](https://www.youtube.com/watch?v=RoKt8mYbJuU&pp=ygUaYWJjIGV4dHJlbWlzbSBHZXJtYW55IGFybXk%3D), [biker gangs](https://www.youtube.com/watch?v=kFiSgLVnGto)
    + *__[Der NSU war nicht zu dritt](https://www.amadeu-antonio-stiftung.de/der-nsu-war-nicht-zu-dritt-31205/)__*

. . . 

- [What is the explanatory power of the [subculture]{style="color:darkred;"} concept? Why is it important?]{style="color:indigo;"} 

## Subculture, radical milieu, and mobilisation - an empirical puzzle

> More than **1000 people** from the Balkan region travelled between 2012 and 2016 to become foreign fighters (Azinović & Bećirević 2017), a disproportionately high number.

- [Why might the Balkans account for a disproportionate (compared to western Europe and to the U.S.) number of ISIS foreign fighters? Let's hear some hypotheses!]{style="color:indigo;"} 

. . . 

- @metodieva2022foreign argues that the **[post-conflict radical milieu]{style="color:darkorange;"}** [is the key factor]{style="color:darkorange;"} [cf. @metodieva2023InfluencesIslamistRadicalization]

# @Bosi2012 - Micro-mobilization into Armed Groups {background-color="#2c844c"}

:::: {.columns}
:::{.column width="50%"}
- levels schema 
- cases and case design 
    + Provos/PIRA and Brigate Rosse 
- paths: ideological, instrumental, solidaristic

:::

::: {.column width="50%" .right}
<div class="tenor-gif-embed" data-postid="16609842" data-share-method="host" data-aspect-ratio="1.79775" data-width="100%"><a href="https://tenor.com/view/police-noob-split-biker-drive-gif-16609842">Police Noob GIF</a>from <a href="https://tenor.com/search/police-gifs">Police GIFs</a></div> <script type="text/javascript" async src="https://tenor.com/embed.js"></script> 
:::
::::

## @Bosi2012[p. 364] levels schema

:::: {.columns}
:::{.column width="50%"}
![](slide_files/05/BosiDellaPortaSchema.png){width="350"}

:::

::: {.column width="50%" .right}
- focus on three central dimensions
    + individual motivations for involvement ([micro]{style="color:blue;"})
    + networks which facilitate the recruitment process ([meso]{style="color:blue;"})
    + effects on individuals of repressive strategies ([macro]{style="color:blue;"})

:::
::::

## @Bosi2012[p. 364] levels schema

:::: {.columns}
:::{.column width="50%"}
![](slide_files/05/BosiDellaPortaSchema_red.png){width="350"}

:::

::: {.column width="50%" .right}
- focus on three central dimensions
    + individual motivations for involvement ([micro]{style="color:blue;"})
    + networks which facilitate the recruitment process ([meso]{style="color:blue;"})
    + effects on individuals of repressive strategies ([macro]{style="color:blue;"})

:::
::::

## Cases and case design [@Bosi2012, pp. 365-366]

- aiming to identify "some of the central paths followed by those individuals who join armed groups" [@Bosi2012, p. 361]
- [paired comparison]{style="color:blue;"} of [most-different cases]{style="color:blue;"} of micro-mobilizations into armed activism with the [aim of singling out some similarities]{style="color:blue;"}

> Our cases represent two of the most influential armed groups in Europe, among the few who lasted for more than 10 years, with a broad territorial coverage and a relatively high number of members and sympathizers. They [differ however in their main ideological roots]{style="color:indigo;"}: ethnonationalist for the PIRA, socio-revolutionary for the BR.

- [thoughts on this ideological division?]{style="color:indigo;"}

## Cases and case design [@Bosi2012, pp. 365-366] {auto-animate=true}

some methodological points: scope for generalisation

- [population]{style="color:blue;"}? 
- [scope conditions]{style="color:blue;"}?

## Cases and case design [@Bosi2012, pp. 365-366] {auto-animate=true}

some methodological points: scope for generalisation

- [population]{style="color:blue;"}? 
    + armed groups (in Europe?)
- [scope conditions]{style="color:blue;"}?
    + groups that lasted more than 10 years
    + broad territorial coverage
    + high number of members and sympathizers

## Cases and case design: generalisation [@Bosi2012, pp. 365-366]

> Although our empirical testing concerns PIRA and BR armed activists, we believe that similar micromobilization paths can be found in other armed groups. This is true if we think of the works of Jocelyan Viterna (2006) on the **[women’s mobilization into the FMLN in El Savador]{style="color:darkorange;"}**, that of Fernando Reinares (2001) with **[ETA militants]{style="color:darkorange;"}**, Olivier Roy’s(2004) work with **[Islamic militants mobilization into Al Qaeda]{style="color:darkorange;"}** in the Middle East, or the work of Gilda Zwerman and Patricia Steinhoff (2005) regarding **[left-wing armed groups in the US and Japan in the post 1960s]{style="color:darkorange;"}**.

## Cases: Provos and BR [@Bosi2012]

- ideologically dissimilar, organisationally similar...
    + [PIRA]{style="color:forestgreen;"}... "*hierarchies of leadership and territorial brigades, battalions, and companies. While the active membership of the organization never exceeded a few hundred at any one time, it is estimated that over 10,000 individuals participated in the PIRA in its almost 30 years of military campaigning*"
    + [BR]{style="color:red;"}... "*organized in city columns, which were subdivided into brigades, with “cells” of three to five members each. Throughout the 1970s and the 1980s it was estimated to have over four hundred full-time members, plus an unknown number of supporters.*"
- the grim tally: [PIRA]{style="color:forestgreen;"} "responsible for over 1,750 deaths between 1970 and 1994"; [BR]{style="color:red;"} "claiming responsibility for 145 killings"

## Paths of radicalisation [@Bosi2012]

| Path | Dominant motivations (micro level) | Recruitment- relevant networks (meso level) | Perception of context (macro level) | 
|-------------:|:-------------|:-------------|:-------------| 
| Ideological  | Ideological, identity  | Family and territorial traditions | Potential revolutionary situation | 
| Instrumental | Aspiration to change   | Political groups | Closed opportunities | 
| Solidaristic | Experiential cognition | Peer group | Escalation of political conflict | 

## Paths of radicalisation [@Bosi2012]

(*[generalizability]{style="color:blue;"}*): [Are the described pathways fitting for less violent groups with more limited goals which never included an attack on the state or the status quo? Can the 3 types be used to analyse counterrevolutionary terrorism]{style="color:indigo;"}?

| Path | Dominant motivations | Recruitment- relevant networks | Perception of context | 
|-------------:|:-------------|:-------------|:-------------| 
| Ideological  | Ideological, identity  | Family and territorial traditions | Potential revolutionary situation | 
| Instrumental | Aspiration to change   | Political groups | Closed opportunities | 
| Solidaristic | Experiential cognition | Peer group | Escalation of political conflict | 
## Ideological path - @Bosi2012[p. 372]

> “We were all trusted comrades: we had known each other for a long time and were very good friends” (Alberto Franceschini, quoted in Fasanella and Franceschini 2004, 44). In other words, in their initial phases both armed groups exploited **[pre-existing social and affective ties]{style="color:darkorange;"}** in recruitment processes in order to **[avoid possible infiltrations]{style="color:darkorange;"}**. While in the Northern Ireland case the **[family]{style="color:darkorange;"}** was also relevant in recruitment to the armed group (e.g. interviewees 1, 6, 11 and 17), **[in the Italian case the family remained more important as a cultural symbol, and joining the BR was perceived as a rupture with the family environment]{style="color:darkorange;"}**

**[examples of this path from extremist group cases?]{style="color:indigo;"}**

## Instrumental path - @Bosi2012[p. 374]

> many activists joined armed groups **[after a long search for effective strategies to achieve their political aims]{style="color:darkorange;"}** (Interviewees 2, 5, 18, 19, 20 for the PIRA and Balzerani, Fiore, Peci, Ronconi and Interviewees 26 and 27 for the BR). They usually joined armed groups later in life **[following dissatisfaction with the “ordinary” politics]{style="color:darkorange;"}** in which they were involved. They judged armed groups according to whether or not they had the capacity to stage successful campaigns and lead to concrete results.

**[examples of this path from extremist group cases?]{style="color:indigo;"}**

## Solidarity path - @Bosi2012[p. 380]

> The **[context]{style="color:darkorange;"}** in which these individuals mobilized was in fact one **[of escalation]{style="color:darkorange;"}**. For many individuals the armed struggle was a way to cope with societies which seemed to them in turmoil. **[High repression and counter-movement violence in collaboration with the establishment]{style="color:darkorange;"}** worked for young individuals as a loss of innocence, which further delegitimized the regimes and justified their mobilization in the PIRA and the BR. ... The leadership of the PIRA had, for example, planned to **[provoke street disturbances with the deliberate intention of producing an outward spiral of violence]{style="color:darkorange;"}**, knowing full well the benefits the British Army repression would reap in terms of support and recruits from the nationalist community.

**[examples of this path from extremist group cases?]{style="color:indigo;"}**

<!-- ([Bosi and Porta, 2012, p. 380](zotero://select/library/items/WQLWKHLS)) SUMMARY OF THREE PATHS   -->
<!-- In the ideological path, we singled out the relevance of deeply rooted family and local traditions, which allowed participants to frame the choice ofjoining armed struggles within a narrative of continuity. The family and the immediate environment provided political socialization into an ideological background in which rebellion could be framed as a sort of obligation in a context perceived as ripe for the successful continuation of the old struggle. Within the instrumental path, mobilization passed instead through the belief that non-violent forms of political protest were no longer helpful in the face of closing political opportunities. They were driven mainly by political grievances. Those who followed this path tended to break more with their past when mobilizing in armed groups in comparison to those who followed an ideological path. A third path developed out of solidarity with a community in struggle in an environment characterized by intense emotions (amongst which anger and revenge were often cited). At least initially, violence was not legitimized by reference to ideology or political strategies, but rather as an everyday element in handling conflicts. It appeared to result from a search for meaning and loyalty to the peer group. -->

# Poll: subculture and recruitment {background-color="#2c844c"}

:::: {.columns}
:::{.column width="35%"}
![](slide_files/05/PVclass5_poll_QR.png){fig-alt="A QR code for the survey."}

:::

::: {.column width="65%" .center}
<!-- go to <https://www.qr-code-generator.com/>, add the link, and screen shot the QR code -->
Take the survey at **<https://forms.gle/oUEhq9W19sBD4E9R9>**

:::
:::: 

::: {style="margin-top: -50px;"}
[[Questions about groups]{style="color:violet;"}: (1) Does a distinct subculture exist around this group? (2) Did participation in the group's social scene (events, meeting places, online spaces, music, symbols, etc.) was an important step toward deeper involvement? (3) Is the group's subculture primarily in-person, online, or mixed? (4) Are most people were likely recruited through pre-existing social ties (e.g., friends, family, classmates)? (5) What do you think was the single most important factor in mobilisation for your group?]{style="font-size: 0.7em;"}

:::

--- 

```{ojs}
md`## Poll results (Respondents: ${respondentCount}) - subculture`
```


```{ojs}

import { liveGoogleSheet } from "@jimjamslam/live-google-sheet";
import { aq, op } from "@uwdata/arquero";

// UPDATE THE LINK FOR A NEW POLL
surveyResults = liveGoogleSheet(
  "https://docs.google.com/spreadsheets/d/e/" +
    "2PACX-1vSSwGAaIviE0BRA9cn-hLDew4_L7tv8HiR9meT_nuRaShGInM75Eeg6pmR6ttTx5QAiJltUL3AHi7OM/" +
    "pub?gid=1280605096&single=true&output=csv",
  10000, 1, 7); // adjust the last number to select all relevant columns 

respondentCount = surveyResults.length;

likertOrder = [
  "Strongly disagree",
  "Disagree",
  "Neutral",
  "Agree",
  "Strongly agree"
];

ynmOrder = [
  "Yes",
  "No",
  "Maybe"
];

groupOrder = [
  "Antifa",
  "RAF",
  "IS",
  "alQaeda",
  "NPD/Heimat",
  "NSU",
  "PKK"
];

groupColors = [
  "pink",
  "red",
  "grey",
  "black",
  "#d2b48c", // lightbrown - could also use 'peru'
  "brown",
  "green"
];

```

:::: {.columns}
:::{.column width="48%"}
Group has a distinct subculture?


```{ojs}
// need to replace 'g_distinct_subculture'

g_distinct_subcultureCounts = aq.from(surveyResults)
  .groupby("g_distinct_subculture", "group")
  .count()
  .objects();

plot_g_distinct_subculture = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
      strokeWidth: 1
    }),
    
    Plot.barY(g_distinct_subcultureCounts, {
      fx: "g_distinct_subculture",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
    })
  ],

  color: {
    domain: groupOrder,
    range: groupColors,
    legend: true,
    style: { fontSize: "20px" }
  },

  fx: {
    domain: ynmOrder,
    label: "",
    tickRotate: 0
  },

  x: {
    domain: groupOrder,
    axis: null, // clears the entire x-axis (ticks and labels)
    // tickFormat: () => "",
    label: ""
  },

  y: {
    label: ""
  },

  marginBottom: 130,
  style: {
    width: 900,
    height: 500,
    fontSize: 20
  }
});

``` 

:::

:::{.column width="4%"}
<div style="height: 600px; border-left: 2px solid #999; margin: 0 auto;"></div>
:::


::: {.column width="48%"}
Subculture is primarily... 

```{ojs}
// need to replace 'g_subculture_primarily'

g_subculture_primarilyCounts = aq.from(surveyResults)
  .groupby("g_subculture_primarily", "group")
  .count()
  .objects();

plot_g_subculture_primarily = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
      strokeWidth: 1
    }),
    
    Plot.barY(g_subculture_primarilyCounts, {
      fx: "g_subculture_primarily",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
    })
  ],

  color: {
    domain: groupOrder,
    range: groupColors,
    legend: true,
    style: { fontSize: "20px" }
  },

  fx: {
    // domain: likertOrder,
    label: "",
    tickRotate: -45
  },

  x: {
    domain: groupOrder,
    axis: null, // clears the entire x-axis (ticks and labels)
    // tickFormat: () => "",
    label: ""
  },

  y: {
    label: ""
  },

  marginBottom: 130,
  style: {
    width: 900,
    height: 500,
    fontSize: 20
  }
});


```

::: 
::::

## Group responses - recruitment

:::: {.columns}
:::{.column width="48%"}
Being in group's social scene precedes membership

```{ojs}
// need to replace 'g_social_participation'

g_social_participationCounts = aq.from(surveyResults)
  .groupby("g_social_participation", "group")
  .count()
  .objects();

plot_g_social_participation = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
      strokeWidth: 1
    }),
    
    Plot.barY(g_social_participationCounts, {
      fx: "g_social_participation",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
    })
  ],

  color: {
    domain: groupOrder,
    range: groupColors,
    legend: true,
    style: { fontSize: "20px" }
  },

  fx: {
    domain: likertOrder,
    label: "",
    tickRotate: -55
  },

  x: {
    domain: groupOrder,
    axis: null, // clears the entire x-axis (ticks and labels)
    // tickFormat: () => "",
    label: ""
  },

  y: {
    label: ""
  },

  marginBottom: 130,
  style: {
    width: 900,
    height: 500,
    fontSize: 20
  }
});

```

:::

:::{.column width="4%"}
<div style="height: 600px; border-left: 2px solid #999; margin: 0 auto;"></div>
:::


::: {.column width="48%"}
Most recruited through existing social ties.

```{ojs}
// need to replace 'g_most_recruitment'

g_most_recruitmentCounts = aq.from(surveyResults)
  .groupby("g_most_recruitment", "group")
  .count()
  .objects();

plot_g_most_recruitment = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
      strokeWidth: 1
    }),
    
    Plot.barY(g_most_recruitmentCounts, {
      fx: "g_most_recruitment",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
    })
  ],

  color: {
    domain: groupOrder,
    range: groupColors,
    legend: true,
    style: { fontSize: "20px" }
  },

  fx: {
    domain: likertOrder,
    label: "",
    tickRotate: -55
  },

  x: {
    domain: groupOrder,
    axis: null, // clears the entire x-axis (ticks and labels)
    // tickFormat: () => "",
    label: ""
  },

  y: {
    label: ""
  },

  marginBottom: 130,
  style: {
    width: 900,
    height: 500,
    fontSize: 20
  }
});

```

::: 
::::

## Group responses - overall importance

single most important factor in mobilisation for the group?

```{ojs}
// need to replace 'g_most_important_factor'

g_most_important_factorCounts = aq.from(surveyResults)
  .groupby("g_most_important_factor", "group")
  .count()
  .objects();

plot_g_most_important_factor = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
      strokeWidth: 1
    }),
    
    Plot.barY(g_most_important_factorCounts, {
      fx: "g_most_important_factor",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
    })
  ],

  color: {
    domain: groupOrder,
    range: groupColors,
    legend: true,
    style: { fontSize: "20px" }
  },

  fx: {
    // domain: likertOrder,
    label: "",
    tickRotate: -45
  },

  x: {
    domain: groupOrder,
    axis: null, // clears the entire x-axis (ticks and labels)
    // tickFormat: () => "",
    label: ""
  },

  y: {
    label: ""
  },

  marginBottom: 130,
  style: {
    width: 900,
    height: 500,
    fontSize: 20
  }
});


```


# Open data - Profiles of Radicalisation {background-color="#ffe0b2"}

::: panel-tabset
### Visualisation 

Recruitment and Organisational Involvement (PIRUS)

```{r warning=FALSE, message=FALSE}
library(tidyverse)
library(plotly)

mobilisation_data <- read_csv("../resources/data/PIRUS_February_2025/week5_pirus_mobilisation.csv")

plot_ly(
  data = mobilisation_data,
  x = ~group_status,
  y = ~count,
  color = ~recruitment,
  type = "bar",
  width = "100%",
  height = "100%",
  hovertemplate = paste(
    "<b>Group Status:</b> %{x}<br>",
    "<b>Recruitment:</b> %{fullData.name}<br>",
    "<b>Count:</b> %{y}<extra></extra>"
  )
) %>% 
  layout(
    barmode = "group",
    xaxis = list(
      title = list(
        text = "Organisational Involvement",
        font = list(size = 22)
      ),
      tickfont = list(size = 18)
    ),
    yaxis = list(
      title = list(
        text = "Number of Individuals",
        font = list(size = 22)
      ),
      tickfont = list(size = 18)
    ),
    legend = list(
      font = list(size = 18)
    ),
    hoverlabel = list(
      font = list(size = 18)
    )
  )

```


### Explanatory note

This visualisation is based on a subset of the [Profiles of Individual Radicalization in the United States](https://www.start.umd.edu/data-tools/profiles-individual-radicalization-united-states-pirus) (PIRUS) dataset. It compares levels of organisational involvement with recruitment experiences and highlights the role of group environments in processes of radicalisation and mobilisation. 

::: {style="text-align: left;"}
**[National Consortium for the Study of Terrorism and Responses to Terrorism (START), University of Maryland. (2024). Profiles of Individual Radicalization in the United States (PIRUS) [Data file]. Retrieved from <http://www.start.umd.edu/data-tools/profiles-individual-radicalization-united-states-pirus>. Copyright University of Maryland 2024.]{style="font-size:18px;"}** 
:::

### Data table 

```{r}
library(reactable)

reactable::reactable(mobilisation_data,
  defaultPageSize = 8,
  style = list(fontSize = "25px"),
  defaultColDef = colDef(
    align = "left",
    minWidth = 50,
    headerStyle = list(background = "#fabf68", fontWeight = "bold") # #f7f7f8
  )
  # ,
  # columns=list(
  #   iyear=colDef(name="Year", align="center", minWidth=50),
  #   gname=colDef(name="Group", minWidth=300), # , maxWidth=200
  #   attacks=colDef(name="Attacks", minWidth=100)
  # )
)
  
```


::: 

## Open data - Profiles of Radicalisation (PIRUS) {background-color="#ffe0b2"}

::: panel-tabset
### Visualisation 


```{r w5-MZ, warning=FALSE, message=FALSE}
library(readxl)

PIRUS <- read_excel("../resources/data/PIRUS_February_2025/PIRUS V5.xlsx")

PIRUS <- PIRUS %>% select(Subject_ID, Year_Exposure, Role_Group, Actively_Recruited, Recruiter1)
## Role_Group: 1 = Loose Associate; 2 = Follower; 3 = Leader; -99 = Unknown; -88 = Not Applicable (individual was not part of a group)
## Actively_Recruited: 0 = No; 1 = Yes; -99 = Unknown
## Recruiter1: 1 = Associate(s) or member(s) of a terrorist or violent extremist group; 2 = Family Member; 3 = Friend; 19; 4 = Other; -99 = Unknown; -88 = N/A (was not recruited)

PIRUS_recoded <- PIRUS %>%
  mutate(
    Role_Group = case_when(
      Role_Group == 1   ~ "Associate",
      Role_Group == 2   ~ "Follower",
      Role_Group == 3   ~ "Leader",
      Role_Group == -88 ~ "NoGroup",
      Role_Group == -99 ~ "Unknown",
      TRUE ~ NA_character_
    ),
    Actively_Recruited = case_when(
      Actively_Recruited == 0   ~ "NotRecruited",
      Actively_Recruited == 1   ~ "Recruited",
      Actively_Recruited == -99 ~ "Unknown",
      TRUE ~ NA_character_
    )
  )

plot_data <- PIRUS_recoded %>%
  select(Year_Exposure, Role_Group, Actively_Recruited) %>%
  pivot_longer(
    cols = c(Role_Group, Actively_Recruited),
    names_to = "Variable",
    values_to = "Category"
  ) %>%
  filter(
    !(Variable == "Role_Group" &
        Category == "Unknown")
  ) %>%
  count(
    Year_Exposure,
    Variable,
    Category,
    name = "N"
  )

# write_csv(plot_data, "PIRUS_plot_data.csv")
```


```{ojs}
pirus = FileAttachment("../resources/data/PIRUS_February_2025/PIRUS_plot_data.csv")
  .csv({typed: true})
  .then(data => data.filter(d => d.Year_Exposure >= 1990))

//pirus = FileAttachment("PIRUS_plot_data.csv").csv({typed: true})
```

::: {style="margin-top: -30px;"}

```{ojs}
viewof vars = Inputs.checkbox(
  ["Role_Group", "Actively_Recruited"],
  {
    label: "Vars.",
    value: ["Role_Group"]
  }
)

plotData = pirus.filter(d => vars.includes(d.Variable))

plotData2 = plotData.map(d => ({
  ...d,
  Group: d.Variable === "Role_Group" ? "Role" : "Recruit"
}))

Plot.plot({
  width: 1800,
  height: 800,
  marginBottom: 100,
  marginLeft: 80,

  style: { fontSize: "30px" },

  x: {
    label: "",
    tickRotate: -90,
    tickFormat: d => String(d)
  },

  y: {
    label: "Count",
    grid: true
  },

  color: {
    legend: true,
    legendFontSize: 30,
    domain: [
      "Associate",
      "Follower",
      "Leader",
      "NoGroup",
      "Recruited",
      "NotRecruited",
      "Unknown"
    ],
    range: [
      "#1b9e77",
      "#d95f02",
      "#7570b3",
      "#666666",
      "#e7298a",
      "#66a61e",
      "#cccccc"
    ]
  },

  marks: [
    Plot.barY(
      plotData2,
      Plot.stackY({
        x: "Year_Exposure",
        y: "N",
        fill: "Category",
        fx: "Group",   // This creates the side-by-side dodge
        tip: true,
        title: d => `${d.Year_Exposure}\n${d.Group}\n${d.Category}\nN = ${d.N}`
      })
    )
  ]
})

```

::: 

### Explanatory note

This visualisation is based on a subset of the [Profiles of Individual Radicalization in the United States](https://www.start.umd.edu/data-tools/profiles-individual-radicalization-united-states-pirus) (PIRUS) dataset. The bar charts show distributions of group roles and recruitment patterns over time.

::: {style="text-align: left;"}
**[National Consortium for the Study of Terrorism and Responses to Terrorism (START), University of Maryland. (2024). Profiles of Individual Radicalization in the United States (PIRUS) [Data file]. Retrieved from <http://www.start.umd.edu/data-tools/profiles-individual-radicalization-united-states-pirus>. Copyright University of Maryland 2024.]{style="font-size:18px;"}** 
:::

### Data table 

```{r}
library(reactable)

reactable::reactable(PIRUS_recoded,
  defaultPageSize = 5,
  style = list(fontSize = "25px"),
  defaultColDef = colDef(
    align = "left",
    minWidth = 50,
    headerStyle = list(background = "#fabf68", fontWeight = "bold") # #f7f7f8
  )
  # ,
  # columns=list(
  #   iyear=colDef(name="Year", align="center", minWidth=50),
  #   gname=colDef(name="Group", minWidth=300), # , maxWidth=200
  #   attacks=colDef(name="Attacks", minWidth=100)
  # )
)
  
```

::: {.callout-note collapse="true" style="margin-top: -15px;"}
#### Data download

```{r echo = F, collapse = TRUE, comment = "#>", message = FALSE, warning=FALSE}
library(downloadthis)

PIRUS_recoded %>% download_this(
    output_name = "PIRUS_recruitment_subset",
    output_extension = ".csv",
    button_label = "Download dataset as csv",
    button_type = "warning",
    has_icon = TRUE,
    icon = "fa fa-save"
  )
```

:::

::: 

## Open data - Profiles of Radicalisation (PIRUS) {background-color="#ffe0b2"}

::: panel-tabset
### Visualisation 

```{r w5-MZ-map, warning=FALSE, message=FALSE}
library(readxl)
library(ggplot2)
library(sf)
library(rnaturalearth)
library(mapview)
library(leafpop)

PIRUS <- read_excel("../resources/data/PIRUS_February_2025/PIRUS V5.xlsx")

PIRUS <- PIRUS %>% select(Subject_ID, Year_Exposure, Role_Group, Actively_Recruited, Recruiter1, Loc_Habitation_State1, Loc_Habitation_City1)
## Role_Group: 1 = Loose Associate; 2 = Follower; 3 = Leader; -99 = Unknown; -88 = Not Applicable (individual was not part of a group)
## Actively_Recruited: 0 = No; 1 = Yes; -99 = Unknown
## Recruiter1: 1 = Associate(s) or member(s) of a terrorist or violent extremist group; 2 = Family Member; 3 = Friend; 19; 4 = Other; -99 = Unknown; -88 = N/A (was not recruited)

PIRUS_recoded <- PIRUS %>%
  mutate(
    Role_Group = case_when(
      Role_Group == 1   ~ "Associate",
      Role_Group == 2   ~ "Follower",
      Role_Group == 3   ~ "Leader",
      Role_Group == -88 ~ "NoGroup",
      Role_Group == -99 ~ "Unknown",
      TRUE ~ NA_character_
    ),
    Actively_Recruited = case_when(
      Actively_Recruited == 0   ~ "NotRecruited",
      Actively_Recruited == 1   ~ "Recruited",
      Actively_Recruited == -99 ~ "Unknown",
      TRUE ~ NA_character_
    ),
    Recruiter1 = case_when(
      Recruiter1 == 1   ~ "Group member",
      Recruiter1 == 2   ~ "Family",
      Recruiter1 == 3   ~ "Friend",
      Recruiter1 == 4   ~ "Other",
      Recruiter1 == -88 ~ "Not recruited",
      Recruiter1 == -99 ~ "Unknown",
      TRUE ~ NA_character_
    ),
    Country = "United States"
  )

## this gets latitude and longitude for the locations, but it takes awhile to
## run (because 1679 distinct locations...)
# PIRUS_geo <- PIRUS_recoded %>% 
#   tidygeocoder::geocode(
#     city = Loc_Habitation_City1,
#     state = Loc_Habitation_State1,
#     country = Country,
#     method = "osm"
#   )

# write_csv(PIRUS_geo, "PIRUS_geo.csv")
PIRUS_geo <- read_csv("../resources/data/PIRUS_February_2025/PIRUS_geo.csv")

us_states <- ne_states(iso_a2="US", returnclass="sf")

bbox <- c(xmin = -127, ymin = 20, xmax = -60, ymax = 50)
us_states_cropped <- st_crop(us_states, bbox)

PIRUS_geo <- PIRUS_geo[!is.na(PIRUS_geo$lat), ]

PIRUS_geo <- PIRUS_geo[!PIRUS_geo$Loc_Habitation_State1 %in% c("Alaska", "Hawaii"), ]

PIRUS_geo_sf <- PIRUS_geo %>% 
  st_as_sf(
    coords = c("long", "lat"),
    crs = st_crs("EPSG:4269") # CRS for US 
  )

st_crs(PIRUS_geo_sf) <- st_crs(us_states_cropped)

PIRUS_geo_sf$Year_chr <- as.character(PIRUS_geo_sf$Year_Exposure)

PIRUS_geo_sf$Decade <- paste0(floor(PIRUS_geo_sf$Year_Exposure / 10) * 10, "s")

leaders   <- subset(PIRUS_geo_sf, Role_Group == "Leader")
followers <- subset(PIRUS_geo_sf, Role_Group == "Follower")
associate <- subset(PIRUS_geo_sf, Role_Group == "Associate")
nogroup   <- subset(PIRUS_geo_sf, Role_Group == "NoGroup")
unknown   <- subset(PIRUS_geo_sf, Role_Group == "Unknown")

mapview(leaders, zcol="Decade", legend=T, layer.name="Leaders",
        label="Role_Group", cex=6,
        map.types=c("CartoDB.Positron","CartoDB.DarkMatter"),
        popup=popupTable(leaders,
                           zcol=c("Year_Exposure","Role_Group","Actively_Recruited",
                                    "Recruiter1","Loc_Habitation_State1",
                                    "Loc_Habitation_City1")))+
  mapview(followers, zcol="Decade", legend=T, layer.name="Followers",
        label="Role_Group", cex=4,
        map.types=c("CartoDB.Positron","CartoDB.DarkMatter"),
        popup=popupTable(followers,
                           zcol=c("Year_Exposure","Role_Group","Actively_Recruited",
                                    "Recruiter1","Loc_Habitation_State1",
                                    "Loc_Habitation_City1")))+
  mapview(associate, zcol="Decade", legend=T, layer.name="Associates",
        label="Role_Group", cex=3,
        map.types=c("CartoDB.Positron","CartoDB.DarkMatter"),
        popup=popupTable(associate,
                           zcol=c("Year_Exposure","Role_Group","Actively_Recruited",
                                    "Recruiter1","Loc_Habitation_State1",
                                    "Loc_Habitation_City1")))+
  mapview(nogroup, zcol="Decade", legend=T, layer.name="No Group",
        label="Role_Group", cex=2,
        map.types=c("CartoDB.Positron","CartoDB.DarkMatter"),
        popup=popupTable(nogroup,
                           zcol=c("Year_Exposure","Role_Group","Actively_Recruited",
                                    "Recruiter1","Loc_Habitation_State1",
                                    "Loc_Habitation_City1")))+
  mapview(unknown, zcol="Decade", legend=T, layer.name="Unknown",
        label="Role_Group", cex=1,
        map.types=c("CartoDB.Positron","CartoDB.DarkMatter"),
        popup=popupTable(unknown,
                           zcol=c("Year_Exposure","Role_Group","Actively_Recruited",
                                    "Recruiter1","Loc_Habitation_State1",
                                    "Loc_Habitation_City1")))

```


### Explanatory note

This visualisation is based on a subset of the [Profiles of Individual Radicalization in the United States](https://www.start.umd.edu/data-tools/profiles-individual-radicalization-united-states-pirus) (PIRUS) dataset. It plots the primary location of radicalised individuals and the bubble reflect their role within groups

compares the number of individuals associated with different ideological categories, including Islamist, White Supremacist, Militia, Anarchist, and Anti-Capitalist movements. The data cover cases recorded between 1948 and 2022 and provide an overview of the ideological diversity represented within the PIRUS database. This allows us to compare the relative prevalence of different ideological backgrounds among radicalised individuals included in the dataset.

::: {style="text-align: left;"}
**[National Consortium for the Study of Terrorism and Responses to Terrorism (START), University of Maryland. (2024). Profiles of Individual Radicalization in the United States (PIRUS) [Data file]. Retrieved from <http://www.start.umd.edu/data-tools/profiles-individual-radicalization-united-states-pirus>. Copyright University of Maryland 2024.]{style="font-size:18px;"}** 
:::

### Data table 

```{r}
library(reactable)

reactable::reactable(PIRUS_geo,
  defaultPageSize = 5,
  style = list(fontSize = "17px"),
  defaultColDef = colDef(
    align = "left",
    minWidth = 50,
    headerStyle = list(background = "#fabf68", fontWeight = "bold") # #f7f7f8
  )
  # ,
  # columns=list(
  #   iyear=colDef(name="Year", align="center", minWidth=50),
  #   gname=colDef(name="Group", minWidth=300), # , maxWidth=200
  #   attacks=colDef(name="Attacks", minWidth=100)
  # )
)
  
```

::: {.callout-note collapse="true" style="margin-top: -15px;"}
#### Data download

```{r echo = F, collapse = TRUE, comment = "#>", message = FALSE, warning=FALSE}
library(downloadthis)

PIRUS_geo %>% download_this(
    output_name = "PIRUS_geo",
    output_extension = ".csv",
    button_label = "Download dataset as csv",
    button_type = "warning",
    has_icon = TRUE,
    icon = "fa fa-save"
  )
```

:::


::: 


# @meier2022SpatiotemporalVariationArmed - Armed Group Recruitment of the LTTE {background-color="#2c844c"}

:::: {.columns}
:::{.column width="50%"}
- (Liberation Tigers of Tamil Eelam)
- Sri Lankan Civil War background

:::

::: {.column width="50%" .right}
<div class="tenor-gif-embed" data-postid="18846127" data-share-method="host" data-aspect-ratio="0.746875" data-width="75%"><a href="https://tenor.com/view/sri-lanka-gif-18846127">Sri Lanka GIF</a>from <a href="https://tenor.com/search/sri+lanka-gifs">Sri Lanka GIFs</a></div> <script type="text/javascript" async src="https://tenor.com/embed.js"></script> 
:::
::::

## @meier2022SpatiotemporalVariationArmed - The Sri Lankan Civil War: Background

- Sri Lankan independence from British Colonial Rule 1948
    + $\rightarrow$ Sinhalese 74%, Tamil 18%  
<!-- - “Ethnic outbidding” (DeVotta 2004) -->
- Inter-generational conflicts within the Tamil Movement
- 1983 anti-Tamil riots as transformative event
- From guerrilla to conventional war 
- The LTTE as de facto state
- 2008 military defeat of the LTTE  

## @meier2022SpatiotemporalVariationArmed - territorial aspect

![](slide_files/05/meier_map.png)

## @meier2022SpatiotemporalVariationArmed - Research interest

- Differential participation in armed groups during civil wars
    + $\rightarrow$ Variation at the sub-national level
    + $\rightarrow$ Temporal and/or spatial variation
- Building on @kalyvas2006logic three different mobilization areas can be distinguished according to the actors in control of the area and the degree to which they exert control
    + Rebel controlled areas
    + Government controlled areas 
    + Contested areas
- $\rightarrow$ Different pathways to militancy within the three mobilization areas? 

## @meier2022SpatiotemporalVariationArmed - Research Process and Methodology

- Selection of research sites
- Interviewing former militants and “similarly placed” civilians
    + $\rightarrow$ [Snowball sampling]{style="color:blue;"} 
- [Life history interviews]{style="color:blue;"}
    + [issues with this approach?]{style="color:indigo;"}

## @meier2022SpatiotemporalVariationArmed - Argument and Contribution to the Literature

- Argument:
    + Depending on the [area where individuals live during civil wars]{style="color:darkorange;"}, they are exposed to [different political orders]{style="color:darkorange;"} and thus experience the conflict in different ways. 
    + Variation in [conflict experience influences people’s decision to take up arms]{style="color:darkorange;"}, resulting in [different individual pathways to militancy]{style="color:darkorange;"} across spatio-temporal contexts 

- Explanatory factors:
    1) actors in control of the area
    2) intensity and form of violence
    3) LTTE recruitment strategy 

## @meier2022SpatiotemporalVariationArmed - Participation Trajectories LTTE Militants 

![](slide_files/05/meier_table.png)

## Open data - racist incidents in Italy {visibility="hidden"}

<!-- {background-color="#ffe0b2"} -->

::: panel-tabset
### Regions map 

```{r it-reg-map, warning=FALSE, message=FALSE}
library(ggplot2)
library(sf)
library(rnaturalearth)
library(mapview)
library(dplyr)
library(leafpop)
library(ggiraph)
library(readxl)

IT_rac <- read_excel("../resources/data/chronik_IT2026-01-31.xlsx")

IT_rac$Latitude <- IT_rac$lat
IT_rac$Longitude <- IT_rac$long

## Regions map ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

IT_reg_grouped <- IT_rac %>% 
  group_by(REGION) %>% 
  summarise(COUNT=n())

IT_reg_grouped <- IT_reg_grouped %>%
  mutate(
    REGION = case_when(
      REGION == "Puglia"       ~ "Apulia",
      TRUE ~ REGION
    )
  )

IT_reg_grouped$TOT <- sum(IT_reg_grouped$COUNT)
IT_reg_grouped$PROP <- IT_reg_grouped$COUNT / IT_reg_grouped$TOT
IT_reg_grouped$PERCENT <- IT_reg_grouped$PROP*100

IT = ne_states(country = "italy", returnclass = "sf")

IT <- IT %>% dplyr::select(province = name, region, geometry)

IT_REGION <- IT %>% group_by(region) %>% summarise(n = n())

map_IT_reg <- IT_REGION %>%
  left_join(IT_reg_grouped, # df.sf
            by = c("region" = "REGION"))

map_IT_reg[is.na(map_IT_reg)] <- 0

plot_map_IT_reg <- ggplot(map_IT_reg) +
  geom_sf(aes(fill = COUNT))+
  geom_sf_text(aes(label=COUNT), colour="red")+
  # geom_sf_text(aes(label = scales::percent(PROP)))+
  scale_fill_gradient("Number of racist incidents", low = "white", high = "gray20")+
  theme_void()+ # theme(legend.position = "none")+
  labs(title="")+theme(plot.title = element_text(hjust = 0.5))
# plot_map_IT_reg

inter_map_IT_reg <- map_IT_reg %>% ggplot(aes(geometry = geometry))+
  geom_sf(aes(fill = PERCENT), color = "black", linewidth = 0.5)+
  geom_sf_interactive(
    fill = NA, 
    aes(
      data_id = region,
      tooltip = glue::glue('{region}<br>2007-(Jan.)2026:<br>{COUNT} incidents')
    ),
    linewidth = 0.1
  )+
  theme_void() +
  scale_fill_gradient(low = "gold", high = "red")+
  theme(
    legend.position = 'none'
  )

girafe(ggobj = inter_map_IT_reg,
       options = list(
         opts_hover(
           css = girafe_css(
             css = '',
             area = 'stroke: black; fill: black;'
           )
         )
       ))

```

### Provinces map 

```{r it-prov-map, warning=FALSE, message=FALSE}

## Provinces map ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

IT_prov_grouped <- IT_rac %>% 
  group_by(PROVINCE) %>% 
  summarise(COUNT=n())

IT_prov_grouped$TOT <- sum(IT_prov_grouped$COUNT)
IT_prov_grouped$PROP <- IT_prov_grouped$COUNT / IT_prov_grouped$TOT
IT_prov_grouped$PERCENT <- IT_prov_grouped$PROP*100

IT_PROV <- IT %>% group_by(province) %>% summarise(n = n())

map_IT_prov <- IT_PROV %>%
  left_join(IT_prov_grouped, # df.sf
            by = c("province" = "PROVINCE"))

map_IT_prov[is.na(map_IT_prov)] <- 0

plot_map_IT_prov <- ggplot(map_IT_prov) +
  geom_sf(aes(fill = COUNT))+
  geom_sf_text(aes(label=COUNT), colour="red")+
  # geom_sf_text(aes(label = scales::percent(PROP)))+
  scale_fill_gradient("Number of racist incidents", low = "white", high = "gray20")+
  theme_void()+ # theme(legend.position = "none")+
  labs(title="")+theme(plot.title = element_text(hjust = 0.5))
# plot_map_IT_prov

inter_map_IT_prov <- map_IT_prov %>% ggplot(aes(geometry = geometry))+
  geom_sf(aes(fill = PERCENT), color = "black", linewidth = 0.5)+
  geom_sf_interactive(
    fill = NA, 
    aes(
      data_id = province,
      tooltip = glue::glue('{province}<br>2007-(Jan.)2026:<br>{COUNT} incidents')
    ),
    linewidth = 0.1
  )+
  theme_void() +
  scale_fill_gradient(low = "gold", high = "red")+
  theme(
    legend.position = 'none'
  )

girafe(ggobj = inter_map_IT_prov,
       options = list(
         opts_hover(
           css = girafe_css(
             css = '',
             area = 'stroke: black; fill: black;'
           )
         )
       ))

```

### Point map

```{r it-point-map, warning=FALSE, message=FALSE, eval=FALSE, include=FALSE}

IT_rac_trim <- IT_rac %>% drop_na(long)
IT_rac_trim <- as.data.frame(IT_rac_trim)

IT_rac_trim$latJIT <- jitter(IT_rac_trim$lat, factor = 5)
IT_rac_trim$longJIT <- jitter(IT_rac_trim$long, factor = 5)

IT_rac_trim_sf_JIT <- IT_rac_trim %>% 
  st_as_sf(
    coords = c("longJIT", "latJIT"),
    crs = st_crs("EPSG:6875") # CRS for Italy
  )

st_crs(IT_rac_trim_sf_JIT) <- st_crs(IT)

IT_rac_trim_sf_JIT <- IT_rac_trim_sf_JIT %>%
  mutate(YEAR = format(DATE, "%Y"),
         DESC_EN_wrap = stringr::str_wrap(DESC_EN, width = 20))

years <- sort(unique(IT_rac_trim_sf_JIT$YEAR))

year_cols <- setNames(
  hcl.colors(length(years), palette = "Dynamic"),
  years
)

IT_by_year <- split(IT_rac_trim_sf_JIT, IT_rac_trim_sf_JIT$YEAR)

mapping <- NULL

for (yr in years) {
  
  m <- mapview(
    IT_by_year[[yr]],
    col.regions = year_cols[yr],
    color = year_cols[yr],
    alpha.regions = 0.7,
    cex = 6,
    label = "LOC",
    legend = FALSE,
    layer.name = yr,
    
    # Only 2026 visible initially
    hide = yr != "2026",
    
    map.types = c(
      "CartoDB.Positron",
      "CartoDB.DarkMatter"
    ),
    
    popup = popupTable(
      IT_by_year[[yr]],
      zcol = c(
        "DATE",
        "LOC",
        "PROVINCE",
        "REGION",
        "REPORT",
        "AREA",
        "DESC_EN"
      )
    )
  )
  
  if (is.null(mapping)) {
    mapping <- m
  } else {
    mapping <- mapping + m
  }
}

mapping

```


### Explanatory note

The visualisations are based on data from 'Lunaria,' an association in Italy that manages an observatory called "[Chronicles of Ordinary Racism](https://www.cronachediordinariorazzismo.org/il-razzismo-quotidiano/?fwp_data=2007-01-04%2C)," in which they collect all discrimination events that occur in Italy, mainly through media reports. 

::: 



# Any feedback for this class? {background-color="#2c844c"}

Anonymous feedback here: <https://forms.gle/NfF1pCfYMbkAT3WP6>

Alternatively, please send me an email: [m.zeller\@lmu.de]{style="color:red;"}


```{r echo=FALSE, message=FALSE, warning=FALSE}
pagedown::chrome_print(file.path("..", "docs/slides", "slides_05.html"))
```

<hr>
    
[Questions about groups next meeting]{style="color:violet;"}: (1) Did the group rely on centralised leadership? (2) Is group leadership based on formal hierarchy (i.e., not informal influence or reputation)? (3) Is the group's network resilient to arrests or leadership loss? (4) How would you describe the group's organisation?  

## References

