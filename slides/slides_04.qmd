...
# Forming presentation groups {background-color="#2c844c"}
...
::: {.column width="50%" .right}
<div class="tenor-gif-embed" data-postid="3578530" data-share-method="host" data-aspect-ratio="1.83908" data-width="100%"><a href="https://tenor.com/view/best-presentation-gif-3578530">That&#39;S The Best Presentation I&#39;Ve Ever Seen GIF</a>from <a href="https://tenor.com/search/best-gifs">Best GIFs</a></div> <script type="text/javascript" async src="https://tenor.com/embed.js"></script> 
:::
::::
...
- most important factor shaping the strategy of a group?
- do maximalist goals require more violent/extreme strategies?
- popular support important for a group to act violently?
...
[[Questions about groups]{style="color:violet;"}: (1) Could the group's goals plausibly be achieved through nonviolence? (2) Would you characterise the group's strategy as mostly nonviolent, mostly violent, or mixed? (3) Who were/are the group's primary targets? (4) Does the group make efforts to avoid civilian casualties? (5) Was the group's use of violence effective, helping to achieve their goals?]{style="font-size: 0.7em;"}
...
Campaign Outcomes by Strategy (NAVCO 2.1)

```{r w4-MZ, warning=FALSE, message=FALSE}
library(tidyverse)
library(plotly)

navco_data <- read_csv("../resources/data/NAVCO_files/week4_navco_strategy_outcomes.csv")
...
This visualisation is based on the [NAVCO 2.1 campaign-year dataset](https://doi.org/10.7910/DVN/MHOXDV), which documents violent and nonviolent resistance campaigns between 1945 and 2013. It compares campaign outcomes across different strategic approaches and highlights the relationship between resistance methods and the achievement of campaign objectives. The data provide insight into how strategic choices shaped the trajectories of mass political movements.

:::

## Open data - Campaign Outcomes (NAVCO) {background-color="#ffe0b2"}

::: panel-tabset
### Visualisation 

```{r w4-MZ2, warning=FALSE, message=FALSE}
library(readxl)

NAVCO <- read_excel("../resources/data/NAVCO_files/NAVCO2-1_ForPublication.xls")

NAVCO <- NAVCO %>% select(camp_name, id, year, prim_meth, success)
## prim_meth: 0=primarily violent campaign 1=primarily nonviolent campaign
...
navco_year <- NAVCO %>%
  mutate(
    violence = if_else(prim_meth == 1, "violent", "nonviolent"),
    outcome  = if_else(success == 1, "successful", "unsuccessful")
  ) %>%
  group_by(year) %>%
  summarise(
    violent_active =
      n_distinct(id[violence == "violent"]),
    nonviolent_active =
      n_distinct(id[violence == "nonviolent"]),
    successful =
      n_distinct(id[outcome == "successful"]),
    unsuccessful =
      n_distinct(id[outcome == "unsuccessful"]),
    .groups = "drop"
  )

navco_year$date <- as.Date(paste0(navco_year$year, "-01-01"))
...
navco_xts <- xts(
  ...
  order.by = navco_year$date
)

## hide the unsuccessful line since it seems to mirror active campaigns
navco_xts <- navco_xts[, c("successful", "violent_active", "nonviolent_active")]

dygraph(navco_xts,
        main = "Active campaigns by type and successes") %>% 
  dyRangeSelector(dateWindow = c("1944-06-01", "2014-06-01")) %>%
  dyBarSeries('successful', color = "forestgreen") %>%
  dySeries("violent_active", strokeWidth = 4, color = "darkred", 
           label = "Violent campaigns", axis = "y2") %>%
  dySeries("nonviolent_active", strokeWidth = 4, color = "skyblue",
           label = "Nonviolent campaigns", axis = "y2") %>%
  dyAxis("y", label = "Campaign outcomes") %>%
  dyAxis("y2", label = "Active campaigns", independentTicks = TRUE) %>%
  dyRangeSelector() %>%
...
NAVCO %>% download_this(
    output_name = "NAVCO_success_subset",
...
## Group responses - strategy

:::: {.columns}
:::{.column width="48%"}
group's goals achievable through nonviolence?

```{ojs}
// need to replace 'g_achieve_with_nonviolence'

g_achieve_with_nonviolenceCounts = aq.from(surveyResults)
  .groupby("g_achieve_with_nonviolence", "group")
  .count()
  .objects();

plot_g_achieve_with_nonviolence = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_achieve_with_nonviolenceCounts, {
      fx: "g_achieve_with_nonviolence",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
::: {.column width="48%"}
characterise the group's strategy?

```{ojs}
// need to replace 'g_characterise_strategy'

g_characterise_strategyCounts = aq.from(surveyResults)
  .groupby("g_characterise_strategy", "group")
  .count()
  .objects();

plot_g_characterise_strategy = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_characterise_strategyCounts, {
      fx: "g_characterise_strategy",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
:::: {.columns}
:::{.column width="48%"}
group's primary targets?

```{ojs}
// need to replace 'g_primary_targets'

g_primary_targetsCounts = aq.from(surveyResults)
  .groupby("g_primary_targets", "group")
  .count()
  .objects();

plot_g_primary_targets = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_primary_targetsCounts, {
      fx: "g_primary_targets",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
::: {.column width="48%"}
Group made efforts to avoid civilian casualties.

```{ojs}
// need to replace 'g_avoid_civ_casualties'

g_avoid_civ_casualtiesCounts = aq.from(surveyResults)
  .groupby("g_avoid_civ_casualties", "group")
  .count()
  .objects();

plot_g_avoid_civ_casualties = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_avoid_civ_casualtiesCounts, {
      fx: "g_avoid_civ_casualties",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
Group's violence was effective, helping to achieve their goals.

```{ojs}
// need to replace 'g_vio_effective'

g_vio_effectiveCounts = aq.from(surveyResults)
  .groupby("g_vio_effective", "group")
  .count()
  .objects();

plot_g_vio_effective = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_vio_effectiveCounts, {
      fx: "g_vio_effective",
      x: "group",
      y: "count",
      fill: "group",
...
```{r ETA, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
library(reshape2)
library(ggplot2)
# library(wesanderson)
...
[Questions about groups next meeting]{style="color:violet;"}: (1) Does a distinct subculture exist around this group? (2) Did participation in the group's social scene (events, meeting places, online spaces, music, symbols, etc.) was an important step toward deeper involvement? (3) Is the group's subculture primarily in-person, online, or mixed? (4) Are most people were likely recruited through pre-existing social ties (e.g., friends, family, classmates)? (5) What was the most important factor in mobilisation for your group?  
