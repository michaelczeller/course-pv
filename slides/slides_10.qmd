...
- violent escalation alienates public, reducing support?
- reason people support politically violent groups?
...
[[Questions about groups]{style="color:violet;"}: (1) Does the group try to grow its popular support? (2) Does the group's violence increase local support for it? (3) What type of participation does the group most rely on? (4) Do most civilians in areas where the group is active support the group? (5) Does the group provide services (security, justice, welfare) that generate legitimacy?]{style="font-size: 0.7em;"}
...
```{ojs}
md`## Group responses (Respondents: ${respondentCount}) - popular support`
```
...
:::: {.columns}
:::{.column width="48%"}
group tries to grow its popular support?


```{ojs}
// need to replace 'g_grow_pop_support'

g_grow_pop_supportCounts = aq.from(surveyResults)
  .groupby("g_grow_pop_support", "group")
  .count()
  .objects();

plot_g_grow_pop_support = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_grow_pop_supportCounts, {
      fx: "g_grow_pop_support",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
::: {.column width="48%"}
group's violence increases local support? 

```{ojs}
// need to replace 'g_vio_increases_support'

g_vio_increases_supportCounts = aq.from(surveyResults)
  .groupby("g_vio_increases_support", "group")
  .count()
  .objects();

plot_g_vio_increases_support = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_vio_increases_supportCounts, {
      fx: "g_vio_increases_support",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
## Group responses - type of participation 

What type of participation does the group most rely on?


```{ojs}
// need to replace 'g_relies_on'

g_relies_onCounts = aq.from(surveyResults)
  .groupby("g_relies_on", "group")
  .count()
  .objects();

plot_g_relies_on = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
...
## Group responses - civilian support 

Do civilians where the group is active support the group? 


```{ojs}
// need to replace 'g_civ_support'

g_civ_supportCounts = aq.from(surveyResults)
  .groupby("g_civ_support", "group")
  .count()
  .objects();

plot_g_civ_support = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
...
## Group responses - service provisions

Does the group provide services (security, justice, welfare) that generate legitimacy?

```{ojs}
// need to replace 'g_provides_services'

g_provides_servicesCounts = aq.from(surveyResults)
  .groupby("g_provides_services", "group")
  .count()
  .objects();

plot_g_provides_services = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_provides_servicesCounts, {
      fx: "g_provides_services",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
<!-- After a major attack, public demand for punishment rises sharply. Should policymakers resist shifting toward harsh policies? -->
<!-- yes/no/maybe -->
...
Average Rebel Strength by Ideology Over Time

```{r w10-local-support, warning=FALSE, message=FALSE}
...
week10 <- ff %>%
  ...
  summarise(
    mean_strength = mean(rebstrength),
    organisations = n(),
    .groups = "drop"
  )
...
plot_ly(
  ...
  text = ~paste(
    "<b>Year:</b>", year,
    "<br><b>Ideology:</b>", ideology,
    "<br><b>Average Rebel Strength:</b>", round(mean_strength,2),
    "<br><b>Number of organisations:</b>", organisations
  ),
  hoverinfo = "text"
) %>%
layout(
  ...
  yaxis = list(
    title = list(
      text="Average Rebel Strength (1=Weak, 5=Strong)",
      font = list(size = 18)
    ),
...
This visualisation is of the [Rebel Foreign Fighter Dataset](https://doi.org/10.1177/00223433231168183) (RFFD), which contains information on insurgent organisations involved in civil wars. It shows how the average strength of rebel organisations has changed over time (between 1989 and 2011) across different ideological movements. The x-axis represents the year, while the y-axis shows the average rebel strength, measured on a scale from 1 (weak) to 5 (very strong). Colours distinguish ideological categories, and hovering over each point reveals the average strength and the number of organisations represented in that year.
...
week10 %>% download_this(
    output_name = "RFFD_rebel_strength",
...
<!-- # some 2025 PV news {background-color="#2c844c" .smaller}  -->
...
<!-- - UK government's use of double agent in northern Ireland conflict [@Carroll2025-12-9]  -->
...
[Questions about groups next meeting]{style="color:violet;"}: (1) Are state responses primarily driven by security concerns (rather than political considerations)? (2) Does the state mostly use policy-based responses? (3) Does the state have a coherent long-term strategy toward the group? (4) How is the group categorised by the state? (5) Has the state engaged the group in negotiation? (6) Do foreign governments shape state responses toward the group?  
