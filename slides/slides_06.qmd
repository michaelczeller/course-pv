...
[[Questions about groups]{style="color:violet;"}: (1) Did the group rely on centralised leadership? (2) Is group leadership based on formal hierarchy (i.e., not informal influence or reputation)? (3) Is the group's network resilient to arrests or leadership loss? (4) How would you describe the group's organisation?]{style="font-size: 0.7em;"}
...
## Open data - Networks of American Radicals (SoNAR) {background-color="#ffe0b2"}

::: panel-tabset
### 2020s Network 

```{r w6-MZ, warning=FALSE, message=FALSE}
library(readxl)
library(dplyr)
library(igraph)
library(visNetwork)
...
SoNAR_Edges <- read_excel("../resources/data/PIRUS_February_2025/SoNAR_Edges.xlsx")
SoNAR_Nodes <- read_excel("../resources/data/PIRUS_February_2025/SoNAR_Nodes.xlsx")

### ISOLATES (NODES WITHOUT CONNECTIONS) INCLUDED

# dec2020s <- "2020s"
# 
# nodes_dec2020s <- SoNAR_Nodes %>% filter(Decade == dec2020s)
# 
# nodes_dec2020s <- nodes_dec2020s %>% filter(!is.na(ID))
# 
# edges_dec2020s <- SoNAR_Edges %>%
#   filter(
#     Source %in% nodes_dec2020s$ID,
#     Target %in% nodes_dec2020s$ID
#   )
# 
# g_2020s <- igraph::graph_from_data_frame(
#   d = edges_dec2020s,
#   vertices = nodes_dec2020s,
#   directed = FALSE
# )
# 
# ideology_cols <- c(
#   "Jihadist" = "gold",                                   
#   "Anarchist/Anti-Capitalist" = "#4575B4",                  # blue
#   "White Supremacist/Xenophobic" = "brown",              
#   "Militia/Sovereign Citizen/Anti-Government" = "darkorange", # orange
#   "Conspiracy Theory" = "#984EA3",                          # purple
#   "Anti-Abortion" = "#F781BF",                              # pink
#   "Other" = "#BDBDBD",                                      # light grey
#   "Black Nationalist/Separatist" = "#000000",              # black
#   "Environmental/Animal Rights" = "#1A9850"                # green
# )
# 
# V(g_2020s)$color <- ideology_cols[V(g_2020s)$Ideology]
# 
# V(g_2020s)$label <- V(g_2020s)$name
# 
# legend_nodes <- data.frame(
#   label = names(ideology_cols),
#   color = unname(ideology_cols),
#   shape = "dot"
# )
# 
# visIgraph(
...
### NO ISOLATES (NODES WITHOUT CONNECTIONS)

dec2020s <- "2020s"

nodes_dec2020s <- SoNAR_Nodes %>%
  filter(
    Decade == dec2020s,
    !is.na(ID)
  )
...
# Remove isolates from node list
connected_ids <- unique(c(edges_dec2020s$Source,
                          edges_dec2020s$Target))

nodes_dec2020s <- nodes_dec2020s %>%
...
ideology_cols <- c(
  "Jihadist" = "gold", 
  "Anarchist/Anti-Capitalist" = "#4575B4",
  "White Supremacist/Xenophobic" = "brown", 
  "Militia/Sovereign Citizen/Anti-Government" = "darkorange", 
  "Conspiracy Theory" = "#984EA3",
  "Anti-Abortion" = "#F781BF", 
  "Other" = "#BDBDBD",
  "Black Nationalist/Separatist" = "#000000", 
  "Environmental/Animal Rights" = "#1A9850" 
)
...
network_data <- read_csv("../resources/data/PIRUS_February_2025/week6_sonar_leadership_networks.csv")
...
plot_ly(
  ...
  hovertemplate = paste(
    "<b>Ideology:</b> %{x}<br>",
    "<b>Average Centrality:</b> %{y}<extra></extra>"
  )
) %>%
  layout(
    title = "Network Centrality Across Ideological Milieus (SoNAR)",
    xaxis = list(title = ""),
    yaxis = list(title = "Average Degree Centrality")
  )
```

### Explanatory note

This visualisation uses data from the Social Networks of American Radicals (SoNAR) project, which maps relationships among extremist offenders in the United States. The network visualisation shows network connections among radicals in the 2020s. The bar chart compares average network centrality across ideological milieus and illustrates differences in the structural positions occupied by actors within extremist networks. Network centrality provides an indication of how connected individuals are within a broader organisational environment and offers insight into patterns of influence, coordination, and leadership.
...
## Open data - Provisional IRA, 1969-1998 {background-color="#ffe0b2"}

::: panel-tabset
### 1969–1976

```{r p1_PIRA, warning=FALSE, message=FALSE}
library(tidyverse)
library(dplyr)
library(igraph)
library(ggraph)
...
p1_att <- read_csv("../resources/data/PIRA_Gill/60_PERIOD1_ATT.csv")
p1_net <- read_csv("../resources/data/PIRA_Gill/60_PERIOD1_NET.csv")

# Node IDs
node_ids <- p1_net$...1

# adjacency matrix to matrix
adj_mat <- p1_net %>%
  select(-...1) %>%
  as.matrix()

rownames(adj_mat) <- node_ids
colnames(adj_mat) <- colnames(p1_net)[-1]
...
# match attributes to network nodes
V(g)$Brigade <- att$Brigade[match(V(g)$name, att$ID)]
...
V(g)$Brigade <- factor(
  V(g)$Brigade,
  levels = names(brigade_cols)
)

V(g)$color <- unname(
  brigade_cols[as.character(V(g)$Brigade)]
)

V(g)$title <- paste0(
  "<b>ID:</b> ", V(g)$name,
  "<br><b>Brigade:</b> ", V(g)$Brigade
)

# V(g)$size <- igraph::degree(g, mode = "all") + 6
...
ggraph(g, layout = "fr") +
  ...
  geom_node_point(
    aes(colour = Brigade), # , size = 3
    size = igraph::degree(g, mode = "all") 
  ) +
  scale_colour_manual(
    values = brigade_cols,
    na.value = "grey70"
...
```{r p2_PIRA, warning=FALSE, message=FALSE}
library(tidyverse)
library(dplyr)
library(igraph)
library(ggraph)
...
p2_att <- read_csv("../resources/data/PIRA_Gill/60_PERIOD2_ATT.csv")
p2_net <- read_csv("../resources/data/PIRA_Gill/60_PERIOD2_NET.csv")
...
rownames(adj_mat) <- node_ids
colnames(adj_mat) <- colnames(p2_net)[-1]
...
```{r p3_PIRA, warning=FALSE, message=FALSE}
library(tidyverse)
library(dplyr)
library(igraph)
library(ggraph)
...
p3_att <- read_csv("../resources/data/PIRA_Gill/60_PERIOD3_ATT.csv")
p3_net <- read_csv("../resources/data/PIRA_Gill/60_PERIOD3_NET.csv")
...
rownames(adj_mat) <- node_ids
colnames(adj_mat) <- colnames(p3_net)[-1]
...
```{r p4_5_PIRA, warning=FALSE, message=FALSE}
library(tidyverse)
library(dplyr)
library(igraph)
library(ggraph)
...
p4_5_att <- read_csv("../resources/data/PIRA_Gill/60_PERIOD4_5_ATT.csv")
p4_5_net <- read_csv("../resources/data/PIRA_Gill/60_PERIOD4_5_NET.csv")
...
rownames(adj_mat) <- node_ids
colnames(adj_mat) <- colnames(p4_5_net)[-1]
...
```{r p6_PIRA, warning=FALSE, message=FALSE}
library(tidyverse)
library(dplyr)
library(igraph)
library(ggraph)
...
p6_att <- read_csv("../resources/data/PIRA_Gill/60_PERIOD6_ATT.csv")
p6_net <- read_csv("../resources/data/PIRA_Gill/60_PERIOD6_NET.csv")
...
rownames(adj_mat) <- node_ids
colnames(adj_mat) <- colnames(p6_net)[-1]
...
This visualisation is of data on active Provisional IRA (hereafter PIRA) members between 1970 and 1998, collected by the International Center for the Study of Terrorism, Pennsylvania State University, and [archived](https://sites.google.com/site/ucinetsoftware/datasets/covert-networks/provisional-irish-republican-army) by The University of Manchester's [Mitchell Centre for Social Network Analysis](https://www.humanities.manchester.ac.uk/research/centres-institutes/mitchell-centre/research/past-projects/covert-networks/outputs/).

Originally published in: Gill, P., Lee, J., Rethemeyer, K. R., Horgan, J., & Asal, V. (2014). [Lethal connections: The determinants of network connections in the Provisional Irish Republican Army, 1970–1998](https://doi.org/10.1080/03050629.2013.863190). *International Interactions, 40*(1), 52-78.
...
## Group responses - leadership 

:::: {.columns}
:::{.column width="48%"}

group relies on centralised leadership

```{ojs}
// need to replace 'g_central_leadership'

g_central_leadershipCounts = aq.from(surveyResults)
  .groupby("g_central_leadership", "group")
  .count()
  .objects();

plot_g_central_leadership = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_central_leadershipCounts, {
      fx: "g_central_leadership",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
::: {.column width="48%"}
Leadership based on formal hierarchy?

```{ojs}
// need to replace 'g_formal_hierarchy'

g_formal_hierarchyCounts = aq.from(surveyResults)
  .groupby("g_formal_hierarchy", "group")
  .count()
  .objects();

plot_g_formal_hierarchy = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_formal_hierarchyCounts, {
      fx: "g_formal_hierarchy",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
## Group responses - org structure

group’s network is resilient to arrests or leadership loss


```{ojs}
// need to replace 'g_network_resilience'

g_network_resilienceCounts = aq.from(surveyResults)
  .groupby("g_network_resilience", "group")
  .count()
  .objects();

plot_g_network_resilience = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
    ...
    Plot.barY(g_network_resilienceCounts, {
      fx: "g_network_resilience",
      x: "group",
      y: "count",
      fill: "group",
      stroke: "white"
...
group's organisation is best described as...

```{ojs}
// need to replace 'g_org_desc'

g_org_descCounts = aq.from(surveyResults)
  .groupby("g_org_desc", "group")
  .count()
  .objects();

plot_g_org_desc = Plot.plot({
  marks: [
    Plot.frame({
      fill: "#f7f7f7",
      stroke: "black",
